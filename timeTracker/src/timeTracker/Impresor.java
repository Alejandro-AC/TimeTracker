...
public class Impresor implements Visitor, Runnable {
     ...
     * @param level: level of the current 
     * Interval in the tree (used for printing).
     */
	public final void visitInterval(final Interval interval, final int level) {
		...
		String format = "  %" + ((level * 2) + 2) + "s%-" 
				+ (formatNumber1 - (level * 2)) 
				+ "s%" + (formatNumber2) + "s%" + (formatNumber2) + "s%" 
				+ (formatNumber3) + "ss%n";	
		
		if (interval.getStartDate() == null || interval.getEndDate() == null) {
			System.out.printf(format, type, 
					interval.getId(), " ", " ", interval.getTotalTime());
		} else {
	...
	public final void visitTask(final Task task, final int level) {
		...
		String type = "T.";
		String format = "  %" + ((level * 2) + 2) + "s%-"  
		+ (formatNumber1 - (level * 2)) + "s%" + (formatNumber2) 
		+ "s%" + (formatNumber2) + "s%" + (formatNumber3) + "ss%n";
		
		if (task.getStartDate() == null || task.getEndDate() == null) {
			System.out.printf(format, type, 
					task.getName(), " ", " ", task.getTotalTime());
		} else {
     ...
     * @param level: level of the current 
     * Project in the tree (used for printing).
     */
	public final void visitProject(final Project project, final int level) {	
		...
		String format = "  %" + ((level * 2) + 2) + "s%-" 
		+ (formatNumber1 - (level * 2)) + "s%" + (formatNumber2) 
		+ "s%" + (formatNumber2) + "s%" + (formatNumber3) 
		+ "ss%n";	
