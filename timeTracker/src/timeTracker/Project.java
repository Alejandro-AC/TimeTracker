...
public class Project extends Activity {
	...
	public final void addChildTask(final Client client) {
		...
		while (!correctType) {
			try {
				...
				getLogger().debug("interval time option introduced" 
						+ " have correct type");
				correctType = true;
			} catch (Exception e) {
				getLogger().warn("introduced value in interval time option" 
		...
		while (!correctOption) {
			if (limitedIntervalTime != 0 && limitedIntervalTime != 1) {
				getLogger().debug("introduced value in " 
		...
		if (limitedIntervalTime == 1) {
			...
			while (!correctType) {
				try {
					...
					getLogger().debug("limited time "
							+ limitedTime + " introduced correctly");
					correctType = true;
				} catch (Exception e) {
					System.out.println("Limited interval time" 
						+	" must be a number.");
					getLogger().warn("Introduced value in" 
						+	" limited interval time is not a number.");
				}
			}
		}
		...
		while (!correctType) {
			try {
				...
				getLogger().debug("scheduled option introduced" 
					+	" have correct type");
				correctType = true;
			} catch (Exception e) {
				getLogger().warn("introduced value in scheduled option" 
					+	" is not a number. Must be 0 or 1.");
				System.out.println("Scheduled option must be 0 or 1." 
		...
		while (!correctOption) {
			if (scheduled != 0 && scheduled != 1) {
				getLogger().debug("introduced value in scheduled option" 
					+	" is not 0 or 1.");
				System.out.println("Scheduled option must be 0 or 1");
				System.out.print("Scheduled (0/1): ");
				scheduled = Integer.parseInt(scanner.nextLine());
		...
		if (scheduled == 1) {
			...
			while (!correctType) {
				try {
					...
					getLogger().debug("introduced date " 
							+ scheduledDate + " correctly");
					correctType = true;
				} catch (Exception e) {
			...
			t = new Scheduled(properties.get(0), properties.get(1),
					this, t, scheduledDate);
		}
	...
	public final ArrayList<String> askChildProperties(final Client client) {
		...
		while (client.getActivity(properties.get(0)) != null) {
			getLogger().warn("activity " + properties.get(0) 
					+ " already exist");
			properties.remove(0);
	...
	public final void calculateTotalTime() {
		...
		getLogger().debug("calculating total time project " 
				+ this.getName() + " = " + this.getTotalTime());
	}
	...
	/**
	 * Function used in the tests to add a new child 
	 * without asking for the properties.
